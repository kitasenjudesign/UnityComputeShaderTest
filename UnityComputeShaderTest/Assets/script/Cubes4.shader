Shader "Cubes4"
{
	Properties
	{
		...
		_ObjectScale("_ObjectScale", Vector) =(0.1,0.1,0.1,1)
	}
	SubShader
	{
		...
		float4 _ObjectScale; // Boidオブジェクトのスケール
		...
		void vert(inout appdata_full v, out Input o)
		{
			...
			float3 scl = _ObjectScale.xyz;          // Boidのスケールを取得
			...
			float4 zeroPos = mul(object2world, float4(0,0,0,1));
			
			float4 projPos = mul( UNITY_MATRIX_VP, zeroPos );
			float4 screenPos = ComputeScreenPos(projPos);
			float2 uv = screenPos.xy/screenPos.w;

			//o.col = tex2Dlod(_MainTex,float4(uv.xy,0,0));//_CubeDataBuffer[unity_InstanceID].color;//色
			o.col = float4(uv,0,0);
		...
		{
			fixed4 c = tex2D (_MainTex, IN.col.xy);// * _Color;
