...
namespace Swarm
{
    public sealed class FloatingSwarm : MonoBehaviour
    {
        ...
        Vector4[] _target;// = new Vector4[_instanceCount];
        Vector4[] _colors;// = new Vector4[_instanceCount];


        Vector4 AttractorVector {
            get {
                var p = _attractor ? _attractor.position : _attractorPosition;
                p = transform.InverseTransformPoint(p);//ワールド空間からローカル空間へ
        ...
        public void ResetPositions()
        {
            if (_positionBuffer != null)
            {
                ...
                _compute.SetBuffer(kernel, "TargetBuffer", _targetBuffer);
        ...
        void Start()
        {
            ...
            _targetBuffer = new ComputeBuffer(InstanceCount, 16);
            _colorBuffer = new ComputeBuffer(InstanceCount, 16);

            _target = new Vector4[_instanceCount];
        ...
        void Update()
        {
            ...
            if(Random.value<0.01f){
                ...
                _colorBuffer.SetData(_colors,0,0,_colorBuffer.count);
                _targetBuffer.SetData(_target,0,0,_targetBuffer.count);
            ...
            if (delta > 0)
            {
                ...
                _compute.SetVector("Attractor", AttractorVector);//attractorの位置渡す
